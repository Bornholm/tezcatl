...
Les marquages `normal`, `ignore`, `symptomatic` et `heartbeat` prennent effet tout de suite et sont persistés avec l'état. Le dernier est le seul qui demande plus de signal, pas moins : il dit « préviens-moi quand cette ligne cesse d'arriver ». La détection apprend une baseline par heure de la journée (`seasonality: hourly`), donc un cron nocturne ou un pic de trafic quotidien ne déclenche rien, alors que le même pic à 3 h du matin est signalé. Le transport accepte `unix://`, `tcp://` et `tls://`, avec certificat côté serveur et `--tls-ca` côté client.
...
La seconde porte sur l'absence, et elle a fini par sortir de la statistique. `disappearance_factor` est relatif, donc trois fois un intervalle moyen de 22 secondes déclenchent au bout de 66 secondes ; un plancher absolu, `disappearance_min_silence`, a réglé ce cas. Mais les scanners SSH de l'instance frappent avec assez de régularité pour passer le filtre du coefficient de variation, et douze minutes de calme sur un intervalle moyen de trois passaient encore. Aucun seuil ne peut distinguer un flux qui manque d'un flux dont personne ne voulait, parce que c'est une question de sens. Un template n'est donc attendu que si quelqu'un l'a marqué `heartbeat`, et son silence porte alors le jugement de cette personne, comme l'apparition d'un template `symptomatic`. `disappearance_scope: all` rétablit l'ancien comportement.

Enfin, `critical` demande une corroboration et pas seulement un écart : deux modalités qui concordent, un changement déclaré juste avant un symptôme dans les logs, ou un jugement humain déjà posé (template symptomatique, seuil configuré, heartbeat qui se tait). Un z-score solitaire, aussi extrême soit-il, s'arrête à `warning`. Un changement ne corrobore qu'un log : le CPU d'un conteneur qui grimpe juste après son propre redémarrage, ou deux conteneurs qui tournent pendant la bascule, c'est le redémarrage lui-même, et le changement explique le chiffre au lieu de l'aggraver. Autrement dit, la statistique seule ne crie jamais.

Les métriques ont leur propre saisonnalité, plus petite que celle des logs. Une série qui s'écarte à la même minute trois jours de suite suit un horaire : sur l'instance de dogfooding, quatre mises à jour de plugins lancées par cron à midi portaient le CPU de l'hôte à 45 % à 12:00:26 trois jours d'affilée, avec des z-scores au-delà de 90 qu'aucune baseline n'apprendrait puisque le reste de la journée est calme. Chaque série se souvient de ses heures d'écart, persistées avec elle, et au bout de `seasonal_days` jours consécutifs l'écart n'est plus remonté. Le jour où il cesse de revenir, il redevient une nouvelle.

Un changement a aussi un sillage. Sur l'instance de dogfooding, un déploiement Dokku faisait parler le noyau, udev, networkd, systemd, docker et la charge de l'hôte, chacun dans son propre événement : 42 des 99 événements de trois jours, jusqu'à 20 pour un seul déploiement. Pendant `echo_window` après un changement, les signaux des autres sources sont repliés dans un seul événement `anomaly.change_echo`, porté par le service qui a changé, plafonné à `warning` et listant ses sources. Ce que quelqu'un a demandé à entendre, un seuil, un template symptomatique, un heartbeat, n'y est jamais replié.
