...
- [x] Poller Prometheus : évaluation périodique de requêtes PromQL (`/api/v1/query`, vector et scalar), nom de métrique logique par requête, identité statique ou dérivée d'un label (`service_label`)
- [x] Activable en standalone comme en serveur — désormais via le **plugin source** `tezcatl-source-prometheus` (`plugins.sources.prometheus` dans la config, ou `tezcatl ingest source prometheus` côté client) ; le poller intégré `metrics.prometheus` a été retiré au profit du plugin (rupture de config assumée pré-1.0, la validation stricte signale le champ disparu)
...
- [x] Watcher de déploiements Kubernetes/Dokku alimentant automatiquement cette modalité — Dokku via le hook post-deploy (paquet tezcatl-dokku), Kubernetes via le plugin source (watch des workloads apps/v1, voir Post-MVP)
...
- [x] Ingestion des métadonnées Kubernetes (déploiements, redémarrages) — plugin source kubernetes, modalité change (voir Post-MVP)
...
- **Système de plugins de sources** ✅ (hashicorp/go-plugin) : un plugin est un binaire `tezcatl-source-<name>` (sous-processus gRPC, contrat `SourceService.Stream` streamant des `Observation` protobuf, config JSON opaque par plugin). SDK public dans `pkg/plugin` (`Serve`, interface `Source`) ; côté hôte, chargement via la config (`plugins.sources`) en serveur/standalone ou via `tezcatl ingest source <name>` en client, avec redémarrage à backoff en cas de crash. Installation depuis GitHub Releases : `tezcatl plugin install github.com/owner/repo` (archives goreleaser, vérification `checksums.txt`), plus `plugin list`/`remove`. Les plugins par défaut vivent dans des **modules Go séparés** (`plugins/*`, `replace` vers la racine) pour ne pas alourdir le go.mod principal — premier livré : `plugins/host` (paquet `tezcatl-plugin-host`, installé dans `/usr/lib/tezcatl/plugins`).

- **Plugin source kubernetes** ✅ (`plugins/kubernetes`, binaire `tezcatl-source-kubernetes`, paquet `tezcatl-plugin-kubernetes`) : events du cluster et logs de pods **directement via l'API server** (HTTP pur, pas de client-go, dans l'esprit du collecteur Docker). Events : list+watch de `/api/v1/events`, chaque occurrence (ADDED et MODIFIED) devient un log au format du tutoriel kubectl (`{time, level, msg}` avec `msg = Kind/name Reason: message`), identité `k8s-events`. Logs : list+watch des pods (les pods créés après le démarrage sont suivis — la limite de `kubectl logs --selector` ne s'applique pas), un stream `follow` par conteneur avec reprise `sinceTime` sur reconnexion ; identité service par labels (`app.kubernetes.io/name`, `app`) puis ownerReferences (hash de ReplicaSet retiré), environnement = namespace par défaut. Changements : list+watch des workloads apps/v1 (Deployments, StatefulSets, DaemonSets), seuls les bumps de `metadata.generation` comptent (le churn de status d'un rollout est ignoré), diff classifié en `deployment` (nouvelle image, version = image raccourcie `checkout:v1.8.2`), `restart` (annotation kubectl rollout restart), `scale` (réplicas) ou `config` ; la liste initiale sert de baseline (pas de replay au démarrage), les re-lists rattrapent ce qui s'est passé pendant une coupure de watch. Auth : `kubeconfig` (contexte au choix, token/tokenFile/certificat client, données inline ou fichiers relatifs — pas les plugins exec), `api_server` explicite (compatible `kubectl proxy`), in-cluster (serviceaccount) autodétecté, puis `$KUBECONFIG`/`~/.kube/config`. Restent pour C4 : métriques.

- **Plugin source prometheus** ✅ (`plugins/prometheus`, binaire `tezcatl-source-prometheus`, paquet `tezcatl-plugin-prometheus`) : le poller PromQL de C2 extrait du cœur en module séparé, même mécanique que host/kubernetes (config JSON : `url`, `interval`, identité par défaut, `queries` avec `service_label`). Le cœur ne compose plus aucune source active : elles sont toutes des plugins. `tezcatl plugin install` accepte désormais un nom de plugin en second argument pour les dépôts qui publient plusieurs archives (`tezcatl plugin install github.com/bornholm/tezcatl prometheus`).
...
- **TUI de diagnostic `tezcatl top`** ✅ (phase 1) : vue interactive façon k9s (tview) sur un serveur en marche (`--target`, rafraîchissement périodique `--refresh`). Vue templates (groupés par partition, gros clusters en tête) avec **marquage au clavier** (`n`/`i`/`s`/`c` sur la ligne sélectionnée — plus besoin de recopier le template exact dans `tezcatl mark`) et détail plein écran (entrée) ; vue metrics triée par écart à la baseline (`|recent−mean|/σ`, warmup en queue) pour calibrer les `min_deltas` d'un coup d'œil ; filtre substring (`/`) sur les deux vues. Phases suivantes : **phase 2** = ring buffer d'événements en mémoire + RPC `StreamEvents` (sink interne de diffusion) pour une vue événements temps réel avec détail et `related_changes` ; **phase 3** = historique des changes, navigation événement→changes, sparklines.
- **Plancher de signification métrique** ✅ (`metrics.detection.min_deltas`) : aucun signal z-score/dérive si l'écart absolu à la baseline est sous un plancher par métrique (nom exact ou glob, défauts `*.percent: 1` et `system.load1: 0.5`). Motivé par le dogfooding Dokku : les conteneurs idle ont une variance quasi nulle, et un CPU passant de 0,03 % à 0,15 % sortait en `critical` (z=7,7) — ~95 % des ~1 900 événements/jour étaient de ce bruit.
...
3. Export/stimulus LLM (le format d'événement actuel en est déjà proche)
4. Voir [PLAN.md](../PLAN.md) pour le reste : traces OTel, détection avancée (ruptures, multivarié), industrialisation (mTLS, multi-tenant, HA) — non pertinents à l'échelle actuelle.
